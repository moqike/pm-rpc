    ...
    function Provider(options) {
        ...
        this._handleMessage = function (event) {
            ...
            if (rpcData.type === types_1.C2P_MESSAGE_TYPE.RPC_REQUEST
                ...
                var deserializedArguments = _this._deserializeArgument(rpcData.arguments);
                var result = _this._invokeService(context_1, method, deserializedArguments);
                _this._sendResult(rpcData.uuid, result);
            }
        };
        this._deserializeArgument = function (argument) {
            var self = _this;
            var result = argument;
            if (Array.isArray(argument)) {
                result = argument.map(_this._deserializeArgument);
            }
                ...
                if (argument && argument.hasOwnProperty(types_1.RPC_ARG_TYPE_KEY)) {
                    switch (argument[types_1.RPC_ARG_TYPE_KEY]) {
                        case types_1.RPC_ARGUMENT_TYPE.CALLBACK:
                            result = function () {
                                ...
                                self._invokeCallback(argument.uuid, __spread(arguments));
                            };
                            break;
                    ...
                    for (var key in argument) {
                        result[key] = _this._deserializeArgument(argument[key]);
                    }
                }
            }
            return result;
